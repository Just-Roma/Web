...
let coeffs=assignCoeffs(300);
const blob = new Blob([document.querySelector('#worker').textContent], { type: "text/javascript" })
let yy;
let qq=performance.now();
Promise.all(Array.from({length: Math.max(navigator.hardwareConcurrency-Math.round(navigator.hardwareConcurrency/4),1)}, () => new Promise((resolve, reject) => {
